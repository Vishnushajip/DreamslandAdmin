...
final agentIdProvider = FutureProvider<String>((ref) async {
  ...
  if (agentId == null || agentId.isEmpty) {
    print('⚠️ Agent ID not found or is empty in SharedPreferences.');
    throw Exception('⚠️ Agent ID not found');
  }
...
class PropertyRepository {
  ...
  Future<List<AgentProperty>> fetchProperties(String agentId) async {
    ...
    if (response.statusCode == 200) {
      ...
      // print('✅ Successfully fetched ${jsonList.length} properties.');
...
final agentPropertiesProvider =
  ...
  // print('📦 Starting to fetch properties...');
